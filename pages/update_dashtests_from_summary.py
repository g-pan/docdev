...
def main():
    ...
    if not dash_table:
        print("ERROR: Could not find table with class 'test-table'")
        sys.exit(1)
    ...
    for idx, data in enumerate(thor_data):
        if data:
            percent = get_percent(data)
            bullet_color = get_bullet_color_standard(percent)
            update_cell_with_bullet(dash_soup, dash_thor_cells[idx+3], data, bullet_color)
    ...
    for idx, data in enumerate(roxie_data):
        if data:
            percent = get_percent(data)
            bullet_color = get_bullet_color_standard(percent)
            update_cell_with_bullet(dash_soup, dash_roxie_cells[idx+3], data, bullet_color)
